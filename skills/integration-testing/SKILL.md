...
description: "Verify code against stateful sandbox twins of the third-party APIs it calls, instead of against mocks or the live vendor. Faults the real service will not produce on demand -- rate limits, timeouts, 5xx, revoked credentials, duplicate or out-of-order webhooks -- can be forced deliberately, and every run prints a receipt of what the sandbox received, so a green carries evidence the traffic happened. The code under test is never edited: veris-proxy reroutes its outbound HTTP(S) from outside the process. Use when writing or changing code that calls an external service; when a failure cannot be reproduced locally; when data silently goes missing, duplicates or arrives out of order; for retry, idempotency, pagination, rate-limit and webhook-delivery behaviour; when no test covers the behaviour, including where the repo has no integration suite; and before concluding from code reading alone that an integration is correct. Call get_environment or get_testing_guide to see which services this account has."
---
